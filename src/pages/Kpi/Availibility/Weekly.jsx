...
import { toMondayISO, dateTimeToISO } from "../../../utils/date";
...
export default function Weekly() {
  ...
  const compute = async () => {
    ...
    try {
      await notify(
        api.post("/api/availability/weekly/compute", { week_start: weekStart, policyId, auto: true }),
        { loading: "Calcul hebdomadaire…", success: "Calcul hebdomadaire terminé ✅" }
      );
      await load();
    } finally {
      setComputing(false);
  ...
  const tauxTone = taux >= 90 ? "success" : taux >= 75 ? "warning" : "danger";

  const exportCsv = async () => {
    if (!weekStart || !policyId) return;
    await notify(
      download("/api/availability/weekly/export", { week_start: weekStart, policyId }),
      { loading: "Export…", success: "Export téléchargé ✅" }
    );
  };
  ...
  return (
    <ComponentCard title="Disponibilité — Hebdomadaire" desc="Calcul et consultation hebdomadaire par règle.">
      {/* Toolbar d’actions */}
      <div className="mb-4 flex flex-wrap items-center justify-end gap-2">
        <Button variant="outline" onClick={compute} disabled={!weekStart || !policyId || computing}>
          <Calculator className={`h-4 w-4 mr-1 ${computing ? "animate-spin" : ""}`} /> Calculer la semaine
        </Button>
        <Button variant="outline" onClick={exportCsv} disabled={!weekStart || !policyId}>
          <FileDown className="h-4 w-4 mr-1" /> Export CSV (avec raisons)
        </Button>
        <Button variant="outline" onClick={load} disabled={!weekStart || !policyId || loading}>
      ...
      <div className="mb-4 grid gap-3 md:grid-cols-12">
        <div className="md:col-span-3">
          <WeekField label="Semaine (lundi)" value={weekStart} onChange={(v) => { setPage(1); setWeekStart(v); }} id="week_start" />
        </div>

        <div className="md:col-span-4">
          <div className="text-xs mb-1 text-gray-500 dark:text-gray-400">Règle de disponibilité</div>
          <PolicySelect value={policyId} onChange={(v) => { setPage(1); setPolicyId(v); }} />
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-4">
        ...
        <KPI label="Disponibles (semaine)" value={fmtInt(availableCount)} tone="success" />
        <KPI label="Indisponibles (semaine)" value={fmtInt(unavailableCount)} tone="danger" />
        <KPI label="Taux disponibilité" value={total ? `${taux.toFixed(1)}%` : "—"} tone={tauxTone} />
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left">
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Semaine (lundi)</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Terminal</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Règle</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Décision</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Jours OK</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Jours KO</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Slots OK</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Slots KO</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Dates KO</th>
                <th className="px-5 py-3 text-gray-500 text-theme-xs whitespace-nowrap">Calculé à</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {tableRows.map((r, idx) => {
                const failDates = Array.isArray(r.fail_dates) ? r.fail_dates.join(", ") : "";
                const isAvailable = String(r.decision).toUpperCase() === "DISPONIBLE" || Number(r.decision) === 1;
                const label = isAvailable ? "DISPONIBLE" : "INDISPONIBLE";
                const tone  = isAvailable ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700";
                return (
                  <tr key={idx}>
                    <td className="px-5 py-3 whitespace-nowrap">{r.week_start}</td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.terminal_sn}</td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.policy_id}</td>
                    <td className="px-5 py-3 whitespace-nowrap">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs ${tone}`}>{label}</span>
                    </td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.days_ok}</td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.days_fail}</td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.slots_ok_total}</td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.slots_fail_total}</td>
                    <td className="px-5 py-3 max-w-[320px] truncate" title={failDates}>{failDates}</td>
                    <td className="px-5 py-3 whitespace-nowrap">{r.computed_at}</td>
                  </tr>
                );
              })}
              {!tableRows.length && (
        ...
        <div className="flex items-center justify-between border-t px-5 py-3 text-sm">
          <div className="text-gray-500">
            Total : <span className="font-medium text-gray-800 dark:text-white">{fmtInt(total)}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" disabled={page * pageSize >= total} onClick={() => setPage((p) => p + 1)}>Suivant</Button>
          </div>
        </div>
      </div>
    </ComponentCard>
