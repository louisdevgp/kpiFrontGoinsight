...
// normalise: retire accents, met en minuscule
...
function FlagChip({ on = 0, label, icon: Icon }) {
  return (
    <span
      ...
      title={`${label} ${on ? "activé" : "désactivé"}`}
    >
...
function PolicyModal({ open, onClose, initial, onSave }) {
  ...
  const [slotHoursStr, setSlotHoursStr] = useState(
    Array.isArray(initial?.slot_hours_json)
      ...
      : DEFAULT_FORM.slot_hours_json.join(",")
  );
  ...
  const errors = useMemo(() => {
    ...
    if (
      !Number.isFinite(form.battery_min_pct) ||
      form.battery_min_pct < 0 ||
      form.battery_min_pct > 100
    ) {
    ...
    if (!parsedSlots.length)
      e.slot_hours_json = "Ajoutez au moins un créneau (heures 0–23).";
    if (!["strict", "lenient"].includes(String(form.paper_mode)))
      e.paper_mode = "Mode papier invalide.";
    return e;
  }, [form, parsedSlots]);
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Statut forcé à <b>Actif</b> — champ masqué.
                </p>
              </div>
            </div>
            <button
            ...
            <div className="mt-5 grid gap-6">
              ...
              <div className="grid gap-4 md:grid-cols-12">
                <div className="md:col-span-10">
                  ...
                  <Input
                    placeholder="Ex: Semaine 44 — Standard"
                    value={form.name}
              ...
              <div className="grid gap-4 md:grid-cols-12">
                <div className="md:col-span-3">
                  ...
                  <Input
                    ...
                    min="0"
                    max="100"
                    value={form.battery_min_pct}
                    onChange={setNum("battery_min_pct")}
                  />
                  {errors.battery_min_pct && (
                ...
                <div className="md:col-span-3">
                  ...
                  <Input
                    type="number"
                    min="0"
                    value={form.daily_fail_N}
                    onChange={setNum("daily_fail_N")}
                  />
                  {errors.daily_fail_N && (
                    <p className="mt-1 text-xs text-red-600">
                      {errors.daily_fail_N}
                ...
                <div className="md:col-span-3">
                  ...
                  <Input
                    type="number"
                    min="0"
                    value={form.weekly_fail_days}
                    onChange={setNum("weekly_fail_days")}
                  />
                  {errors.weekly_fail_days && (
                    <p className="mt-1 text-xs text-red-600">
                      {errors.weekly_fail_days}
                    </p>
                  )}
                ...
                <div className="md:col-span-3">
                  ...
                  <Input
                    type="number"
                    min="0"
                    value={form.weekly_fail_slots}
                    onChange={setNum("weekly_fail_slots")}
                  />
                  {errors.weekly_fail_slots && (
                    <p className="mt-1 text-xs text-red-600">
                      {errors.weekly_fail_slots}
                    </p>
                  )}
              ...
              <div>
                <div className="text-xs mb-1 text-gray-500">
                  Créneaux (heures séparées par des virgules)
                </div>
                <Input
                ...
                <div className="mt-2">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => setSlotHoursStr(DEFAULT_SLOTS.join(","))}
                  >
              ...
              <div className="grid gap-6 md:grid-cols-12">
                <div className="md:col-span-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, paper_mode: e.target.value }))
                    }
                  >
                    <option value="strict">
                      Strict (absence d’info = non conforme)
                    </option>
                    <option value="lenient">
                      Souple (absence d’info = tolérée)
                    </option>
                  </select>
                ...
                <div className="md:col-span-8">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={!!form.use_internet}
                        onChange={toggle("use_internet")}
                      />
                    ...
                    <label
                      className="flex items-center gap-2 text-sm"
                      title="TPE allumé / actif"
                    >
                      <input
                        type="checkbox"
                        checked={!!form.use_tpe_on}
                        onChange={toggle("use_tpe_on")}
                      />
                      TPE allumé
                    </label>
                    <label
                      className="flex items-center gap-2 text-sm"
                      title="Position dans la zone définie"
                    >
                      <input
                        type="checkbox"
                        checked={!!form.use_geofence}
                        onChange={toggle("use_geofence")}
                      />
                      <span className="inline-flex items-center gap-1">
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={!!form.use_battery}
                        onChange={toggle("use_battery")}
                      />
                      <span className="inline-flex items-center gap-1">
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={!!form.use_printer}
                        onChange={toggle("use_printer")}
                      />
                      <span className="inline-flex items-center gap-1">
                    ...
                    <label
                      className="flex items-center gap-2 text-sm"
                      title="Présence de papier"
                    >
                      <input
                        type="checkbox"
                        checked={!!form.use_paper}
                        onChange={toggle("use_paper")}
                      />
                      Papier
                    </label>
                  </div>
...
export default function Policies() {
  ...
  }, [page, pageSize /* on ne dépend pas de search pour éviter sur-fetch */]);

  /* ---------- Recherche locale garantie (Nom + ID, accents ignorés) ---------- */

  const filteredRows = useMemo(() => {
    if (!search) return rows;
    const q = normalizeTxt(search);
    return rows.filter((r) => {
      const name = normalizeTxt(r?.name);
      const idStr = normalizeTxt(r?.id);
  ...
  const tableRows = useMemo(() => {
    if (!serverPaged) {
      ...
      // met à jour le total affiché avec le nombre filtré
      setTotal(filteredRows.length);
      return slice;
    }
    // mode serveur : on affiche la page reçue (mais déjà filtrée localement si besoin)
    return filteredRows;
  }, [filteredRows, serverPaged, page, pageSize]);
  ...
  const onSaveModal = async (payload, id) => {
    ...
    if (id) {
      await notify(updatePolicy(id, forced), {
        loading: "Mise à jour…",
        success: "Règle mise à jour",
      });
    } else {
      await notify(createPolicy(forced), {
        loading: "Création…",
        success: "Règle créée",
      });
    }
  ...
  return (
    ...
    <div className="space-y-6">
      <ComponentCard
        ...
        rightSlot={
          <div className="flex items-center gap-2">
            <Button type="button" onClick={openCreate} title="Créer une nouvelle règle">
              <Plus className="h-4 w-4 mr-1" /> Nouvelle politique
            </Button>
            <Button type="button" variant="outline" onClick={load} title="Rafraîchir la liste">
        ...
        <div className="mb-4 grid gap-3 md:grid-cols-12">
          <div className="md:col-span-8">
            <div
              ...
              title="Rechercher par nom de règle ou ID"
            >
              Recherche (nom ou ID)
            </div>
            <div className="flex gap-2">
              <Input
                placeholder="Ex: Semaine 44 ou 102"
                value={search}
                onChange={(e) => {
                  setPage(1);
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    setPage(1);
                    // si tu veux forcer un fetch serveur: load();
                  }
                }}
                aria-label="Recherche (nom ou ID)"
              />
              {search && (
                <Button
                  variant="outline"
              ...
              <Button
                ...
                onClick={() => {
                  setPage(1);
                  // Optionnel: si l'API sait filtrer côté serveur, dé-commente:
                  // load();
                }}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-left">
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap w-16">
                    ID
                  </th>
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap">
                    Nom de la règle
                  </th>
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap w-24">
                    Statut
                  </th>
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap w-24 text-right">
                    Batt. (%)
                  </th>
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap w-24 text-right">
                    Éch./j
                  ...
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap w-28 text-right">
                    Créneaux KO
                  </th>
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap">
                    Créneaux évalués
                  ...
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap w-28">
                    Mode papier
                  </th>
                  <th className="px-4 py-3 text-gray-500 text-theme-xs whitespace-nowrap text-right w-28">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                ...
                {!loading &&
                  tableRows.map((r) => {
                    const slots = Array.isArray(r.slot_hours_json)
                    ...
                    const paperMode =
                      r.paper_mode === "strict"
                        ? "Strict"
                        : r.paper_mode === "lenient"
                        ? "Souple"
                        : "—";
                    return (
                      <tr key={r.id}>
                        <td className="px-4 py-3 whitespace-nowrap w-16 tabular-nums">
                          {r.id}
                        </td>
                        <td
                        ...
                        <td className="px-4 py-3 whitespace-nowrap w-24 text-right tabular-nums">
                          {r.battery_min_pct}%
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap w-24 text-right tabular-nums">
                          {r.daily_fail_N}
                        ...
                        <td className="px-4 py-3 whitespace-nowrap w-28 text-right tabular-nums">
                          {r.weekly_fail_slots}
                        </td>
                        <td
                          className="px-4 py-3 max-w-[260px] truncate"
          ...
          <div className="flex items-center justify-between border-t px-5 py-3 text-sm">
            <div className="text-gray-500">
              ...
              <span className="font-medium text-gray-800 dark:text-white">
                {serverPaged ? filteredRows.length : total}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Button
              ...
              <Button
                variant="outline"
                type="button"
                disabled={page * pageSize >= (serverPaged ? filteredRows.length : total) || loading}
